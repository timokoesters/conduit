        ...
        if okay {
            ...
            for state in fork_states {
                auth_chain_sets.push(
                    get_auth_chain(&room_id, state.iter().map(|(_, id)| id.clone()).collect(), db)
...
fn get_auth_chain_inner(room_id: &RoomId, event_id: &EventId, db: &Database) -> Result<HashSet<u64>> {
    ...
    while i < queued_events.len() && events.len() < u64::from(body.limit) as usize {
        if let Some(pdu) = db.rooms.get_pdu_json(&queued_events[i])? {
            ...
            if event_room_id != body.room_id {
                warn!("Evil event detected: Event {} found while searching in room {}", queued_events[i], body.room_id);
...
async fn create_join_event(
    ...
    let auth_chain_ids = get_auth_chain(&room_id, state_ids.iter().map(|(_, id)| id.clone()).collect(), &db)?;
